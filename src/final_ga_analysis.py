...
DATA_FILE = PROJECT_DIR / "data" / "HOPE_측정자료_상황별정리.xlsx"
...
def regression_line(ax, x: np.ndarray, y: np.ndarray, color: str, style: str) -> None:
    ...
    ax.plot(xx, slope * xx + intercept, color=color, linestyle=style, linewidth=2.4, label="Linear Regression")


def format_axis(ax, ylabel: str) -> None:
    ax.set_xlabel("Upload + Download (Mbps)")
...
PANEL_TITLES = [
    [
        "AP Scores under RSSI Variation\n(Conventional RSSI)",
        "AP Scores under RSSI Variation\n(APQI)",
        "AP Scores under RSSI Variation\n(GA)",
    ],
    [
        "AP Scores in a Good-RSSI, Congestion-Dominant Environment\n(Conventional RSSI)",
        "AP Scores in a Good-RSSI, Congestion-Dominant Environment\n(APQI)",
        "AP Scores in a Good-RSSI, Congestion-Dominant Environment\n(GA)",
    ],
    [
        "AP Scores in a Mixed Environment\n(Conventional RSSI)",
        "AP Scores in a Mixed Environment\n(APQI)",
        "AP Scores in a Mixed Environment\n(GA)",
    ],
]
...
def draw_ga_training(
    ...
    axes[0].set_title("GA Weights for the Congestion Score")
    axes[0].grid(axis="y", alpha=0.22)

    axes[1].plot(history["Generation"], history["Best_Pearson_r"], label="Best Fitness", color="#2A6FBB")
    axes[1].plot(history["Generation"], history["Population_Mean_r"], label="Population Mean", color="#999999", alpha=0.75)
    ...
    names = ["Equal Weights", "GA Training", "5-Fold OOF", "Session OOF"]
    ...
    axes[2].set_title("Overall Hybrid Correlation")
    axes[2].grid(axis="y", alpha=0.22)

    fig.tight_layout()
    fig.savefig(FIGURES_DIR / "ga_training_result.png", dpi=240, bbox_inches="tight")
    plt.close(fig)
...
def draw_nine_panel(
    ...
    for row, environment in enumerate(["RSSI 변화 환경", "RSSI 양호·혼잡 환경", "혼합환경"]):
        ...
        ax.scatter(actual, rssi_score, s=38, alpha=0.70, color="#EF6A5B", edgecolors="white", linewidths=0.3,
                   label=f"Valid Samples: {len(frame)}")
        ...
        format_axis(ax, "AP Score")

        ax = axes[row, 1]
        ax.scatter(actual[qualified], apqi_score[qualified], s=39, alpha=0.74, color="#7B61A8",
                   edgecolors="white", linewidths=0.3, label=f"Candidates Passed: {int(qualified.sum())}")
        ax.scatter(actual[~qualified], apqi_score[~qualified], s=43, alpha=0.78, color="#F28E2B",
                   marker="x", linewidths=1.2, label=f"Threshold Rejected: {int((~qualified).sum())}")
        ...
        ax.scatter(actual[normal], ga_score[normal], s=38, alpha=0.72, color="#42A5E5",
                   edgecolors="white", linewidths=0.3, label=f"Normal Mode (RSSI): {int(normal.sum())}")
        ax.scatter(actual[congested], ga_score[congested], s=43, alpha=0.76, color="#F28E2B",
                   marker="D", edgecolors="white", linewidths=0.3,
                   label=f"Congested Mode (RTT + UTIL): {int(congested.sum())}")
    ...
    fig.savefig(FIGURES_DIR / "method_comparison.png", dpi=230, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    history.to_csv(TABLES_DIR / "ga_convergence.csv", index=False, encoding="utf-8-sig")
    qc.to_csv(TABLES_DIR / "state_counts.csv", index=False, encoding="utf-8-sig")
    row_results.to_csv(TABLES_DIR / "row_level_scores.csv", index=False, encoding="utf-8-sig")
    cv_results.to_csv(TABLES_DIR / "five_fold_results.csv", index=False, encoding="utf-8-sig")
    ga_comparison.to_csv(TABLES_DIR / "weight_comparison.csv", index=False, encoding="utf-8-sig")
    excluded_frame.to_csv(TABLES_DIR / "excluded_rows.csv", index=False, encoding="utf-8-sig")
    session_cv_results.to_csv(TABLES_DIR / "session_holdout_results.csv", index=False, encoding="utf-8-sig")
    grid_results.to_csv(TABLES_DIR / "grid_search_results.csv", index=False, encoding="utf-8-sig")
    paper_metrics.to_csv(TABLES_DIR / "paper_metrics.csv", index=False, encoding="utf-8-sig")
    metrics.to_csv(TABLES_DIR / "performance_metrics.csv", index=False, encoding="utf-8-sig")
    ...
    (RESULTS_DIR / "run_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    metric_lines = [
    ...
    (RESULTS_DIR / "summary.txt").write_text(readable, encoding="utf-8")
